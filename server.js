...
1. 「すみません、予約専用のAIアシスタントが連絡させていただいております。予約をお願いしたいのですが」とだけ言って、相手の反応を待つ。ここで予約の詳細を言うな。
...
1. "안녕하세요, 예약 전용 AI 어시스턴트가 연락드립니다. 예약을 하고 싶은데요"라고만 말하고 상대방의 반응을 기다려라. 여기서 예약 상세를 말하지 마라.
...
1. Greet in ${langName} and say "This is an AI assistant dedicated to reservations calling on behalf of a customer. I'd like to make a reservation." Wait for their response. Do NOT give details yet.
    ...
    function connectOpenAI() {
      ...
      openaiWs.on('open', () => {
        ...
        const sendGreeting = () => {
          ...
          if (callLang === 'ja') {
            instr = isReservation
              ? '次のセリフだけ話せ: すみません、予約専用のAIアシスタントが連絡させていただいております。予約をお願いしたいのですが。 ― 一字一句このまま話せ。省略するな。追加するな。'
              : '極めて早口で「あの、すみません。今営業されてますか？」とだけ言え。余計な前置き不要。間を空けるな。一切の遅延なく話し始めろ。';
          } else if (callLang === 'ko') {
            instr = isReservation
              ? '다음 대사만 말해라: 안녕하세요, 예약 전용 AI 어시스턴트가 연락드립니다. 예약을 하고 싶은데요. ― 한 글자도 빠짐없이 그대로 말해라. 생략하지 마라. 추가하지 마라.'
              : '매우 빠르게 "안녕하세요, 지금 영업 중이신가요?"라고만 말해라. 불필요한 서두 없이. 즉시 말하기 시작해라.';
          } else {
            const ln = getLangName(callLang);
            instr = isReservation
              ? `Speak ONLY in ${ln}. Say this very quickly and naturally: "Hello, this is an AI assistant dedicated to reservations calling on behalf of a customer. I'd like to make a reservation." Nothing else. Do not give any details yet.`
          ...
          if (callMode === 'reservation' && !greetingDone && !farewellSent) {
            if (!greetingAudioSent && greetingRetryCount < 2) {
              ...
              setTimeout(() => {
                ...
                let retryInstr;
                if (callLang === 'ja') retryInstr = '次のセリフだけ話せ: すみません、予約専用のAIアシスタントが連絡させていただいております。予約をお願いしたいのですが。 ― 一字一句このまま話せ。省略するな。追加するな。';
                else if (callLang === 'ko') retryInstr = '다음 대사만 말해라: 안녕하세요, 예약 전용 AI 어시스턴트가 연락드립니다. 예약을 하고 싶은데요. ― 한 글자도 빠짐없이 그대로 말해라. 생략하지 마라. 추가하지 마라.';
                else retryInstr = `Speak ONLY in ${getLangName(callLang)}. Say: "Hello, this is an AI assistant dedicated to reservations calling on behalf of a customer. I'd like to make a reservation." Nothing else.`;
                openaiWs.send(JSON.stringify({
                  type: 'response.create',
                  response: {
                    modalities: ['text', 'audio'],
